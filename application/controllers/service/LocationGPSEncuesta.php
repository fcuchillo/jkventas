...
class LocationGPSEncuesta extends REST_Controller {                                                                                
    ...
    public function UserLocationGPSEncuesta_post(){
       
        $tablet=$this->input->post('tabletId');
         //echo json_encode($tablet);
        
        if($tablet!=null){
                $get_data_bd=$this->getExistsUserIntoDataBase($tablet);
                
               //echo json_encode($get_data_bd);
               if($get_data_bd!=null){
                    $data = array(
                        'USUARIO_ID'    => $get_data_bd[0]->USUARIO_ID,
                        'CARGO_ID'      => $get_data_bd[0]->CARGO_ID,
                        'EQUIPO_ID'     => $get_data_bd[0]->EQUIPO_USU,
                        ...
                        'NSELV'         => $this->input->post('vivienda'),
                   ...
                   $this->db_encuesta_captura->insert('T_ENC_CAPT_GEOLOCALIZACION', $data);
